...
#install.packages('rlang') # need rlang version of 1.0.6+
library(igraph)
library(ggraph)
...
nhood_conn = read.csv('csv/nhood_conn.csv', row.names = 1)
layout = data.matrix(read.csv('csv/layout.csv', row.names = 1))
obs = read.csv('csv/nhood_obs.csv', row.names = 1)
...
colour_by <- factor(colour_by, levels = c('DP(P)_T','DP(Q)_T','ABT(ENTRY)','CD8+T','CD4+T'))

# plotting
pdf(paste0('/home/jovyan/mount/gdrive/VDJ_collab/plots_output/chenqu_jhub/',"nhood_umap.pdf"),width=6, height=4)
...
# PCA
layout_vdj_pca = data.matrix(read.csv('csv/layout_vdj_pca.csv', row.names = 1))

pdf(paste0('/home/jovyan/mount/gdrive/VDJ_collab/plots_output/chenqu_jhub/',"nhood_vdj_pca.pdf"),width=6, height=4)
...
# UMAP
layout_vdj_umap = data.matrix(read.csv('csv/layout_vdj_umap.csv', row.names = 1))

pdf(paste0('/home/jovyan/mount/gdrive/VDJ_collab/plots_output/chenqu_jhub/',"nhood_vdj_umap.pdf"),width=6, height=4)
